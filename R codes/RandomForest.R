...
n_varibles_in_impt_plots <- 28 # choose number of variables to show plots
...
legend("topright",
legend = c("OOB", groups), col = c("black", group_color), lty = 1) # legends
...
p_min_depth <- plot_min_depth_distribution(min_depth, mean_sample = "top_trees")+
  theme(text = element_text(size = 18), axis.text = element_text(size = 18)) + 
  scale_fill_manual(values = colorRampPalette(c("#a8ddb5", "gray70"))(ncol(my_data)-1))
p_min_depth
...
diag(x)=NA
dev.new()
pheatmap(x, cluster_cols = FALSE, cluster_rows = FALSE, angle_col = 0, labels_row = c("Control", "Mild", "Severe", "Oxygen"), labels_col = c("Control", "Mild", "Severe", "Oxygen"), display_numbers = TRUE, color = colorRampPalette(c("#a8ddb5", "#fb6a4a", "#b10026"))( 10 ), fontsize = 18, na_col = "white", border_color = "white" )
...
diag(x)=NA
dev.new()
pheatmap(x, cluster_cols = FALSE, cluster_rows = FALSE, angle_col = 0, labels_row = c("G1", "G2", "G3"), labels_col = c("G1", "G2", "G3"), display_numbers = TRUE, color = colorRampPalette(c("#a8ddb5", "#fb6a4a", "#b10026"))( 10 ), fontsize = 30, na_col = "white", border_color = "white" )
